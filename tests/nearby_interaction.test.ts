...
function rig(targets: Entity[] = [], nodes: GatherNodeDef[] = []) {
  ...
  const world = {
    ...
    casinoInteract: (id: number) => {
      calls.push(`casino:${id}`);
    },
...
describe('tryNearbyInteraction', () => {
  ...
  it.each([
    ...
    [
      'riverboat gangway',
      entity({ id: 2, kind: 'object', templateId: 'riverboat_gangway', lootable: true }),
      'casino:2',
    ],
    [
      'riverboat exit',
      entity({ id: 2, kind: 'object', templateId: 'riverboat_exit', lootable: true }),
      'casino:2',
    ],
  ...
  it.each([
    ...
    ['riverboat croupier', 'riverboat_hilo_croupier', 'casino:2'],
