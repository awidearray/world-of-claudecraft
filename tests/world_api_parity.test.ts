...
// it lands the matching edit here in the SAME commit. The count pins (206 / 54 / 152)
...
// Partition: 54 `data` + 152 `method` (read-returning + command-void + async).
...
export const IWORLD_MEMBERS = [
  ...
  { name: 'realmCurrency', kind: 'method' }, // read-returning (phase 7 currency identity)
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 206', () => {
  ...
  it('the sorted method-kind set is exactly the pinned 152', () => {
...
//   (4) their union, sorted, equals the pinned 206-name IWORLD_MEMBERS set (a member
