...
describe('computeReferralBonus', () => {
  ...
  it('is all zero outside the window', () => {
    expect(computeReferralBonus(1000, 2000, false)).toEqual({
      refereeXp: 0,
      refereeCopper: 0,
      referrerXp: 0,
      referrerCopper: 0,
  ...
  it('is zero for no new earnings, and clamps negative deltas', () => {
    expect(computeReferralBonus(0, 0, true)).toEqual({
      refereeXp: 0,
      refereeCopper: 0,
      referrerXp: 0,
      referrerCopper: 0,
    });
    expect(computeReferralBonus(-50, -10, true)).toEqual({
      refereeXp: 0,
      refereeCopper: 0,
      referrerXp: 0,
      referrerCopper: 0,
...
describe('reconcileReferral', () => {
  ...
  it('does NOT re-count a prior boost on the next round (no compounding)', () => {
    ...
    const r2 = reconcileReferral({
      xpGained: 1600,
      lootCopper: 0,
      lastXpGained: r1.newLastXpGained,
      lastLootCopper: 0,
      withinWindow: true,
    });
  ...
  it('out of window: no grant, but the checkpoint still advances to current counters', () => {
    ...
    expect(r).toMatchObject({
      ...
      referrerCopper: 0,
      applied: false,
      newLastXpGained: 5000,
      newLastLootCopper: 3000,
    });
  });
});
