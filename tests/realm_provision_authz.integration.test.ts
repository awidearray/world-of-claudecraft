...
run('stake-to-provision authorization + guards (real Postgres)', () => {
  ...
  describe('prepareProvisionQuote name + cap guards (pre-RPC)', () => {
    it('rejects an invalid realm name (charset/length)', async () => {
      const r = await provision.prepareProvisionQuote(db.pool, {
        accountId: accA,
    ...
    it('rejects once the per-account realm cap is reached', async () => {
      ...
      for (let i = 0; i < cap; i++) {
        await realmDb.createProvisioningRealm(db.pool, {
          name: `CapRealm ${capAcc} ${i}`,
          type: 'Normal',
          ownerAccountId: capAcc,
          tier: 1,
        });
      }
      const r = await provision.prepareProvisionQuote(db.pool, {
  ...
  describe('confirmProvisionQuote authorization', () => {
    it('404s an unknown quote', async () => {
      const r = await provision.confirmProvisionQuote(db.pool, {
        accountId: accA,
    ...
    it('403s when another account tries to confirm your quote', async () => {
      const realmId = (
        await realmDb.createProvisioningRealm(db.pool, {
          name: `QuoteRealm ${Date.now()}`,
          type: 'Normal',
          ownerAccountId: accA,
          tier: 1,
        })
      ).realmId;
      await quoteDb.createRealmQuote(db.pool, {
        quoteId: 'q-owned-by-a',
        accountId: accA,
        realmId,
        ownerWallet: WALLET_A,
        amountBase: 100n,
  ...
  describe('requestRealmDecommission authorization + state', () => {
    it('404s an unknown realm', async () => {
      const r = await provision.requestRealmDecommission(db.pool, {
        accountId: accA,
        realmId: 999999,
      });
