...
- Webhooks relay raw through the BFF (the service is not publicly reachable): `POST /api/merch/stripe/webhook` and `POST /api/merch/printful/webhook` forward body + signature header verbatim; the service verifies both signatures.
- Fail-closed: flag off or service dark means the server behaves byte-identically, the tab stays hidden, and every proxy call resolves to its typed unavailable result - never a throw.
...
- Rate-limit + short-cache `GET /api/merch/products` (public and proxied), like `/api/woc/balance`?
