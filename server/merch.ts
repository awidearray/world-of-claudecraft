...
/** Whether the merch store env is configured (does not confirm reachability). */
export function merchConfigured(): boolean {
  return merchServiceConfigured();
}
...
function merchHandler(ctx: Ctx): Promise<void> {
  return handleMerchApi(ctx.req, ctx.res, ctxAccountId(ctx));
}
