...
import { REALM_POWER_SCHEMA } from './realm_power_db';
...
export async function ensureSchema(): Promise<void> {
  ...
  try {
    ...
    await client.query(REALM_POWER_SCHEMA); // power-credit quotes + ledger (launchpad phase 7, references realms)
...
export async function getPlayerCardMetaBySlug(slug: string): Promise<{
  ...
  const res = await pool.query(
    `SELECT pc.title, pc.description, pc.locale, pc.updated_at, c.name AS character_name
