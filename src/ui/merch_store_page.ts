...
import { buildMerchView, type MerchView } from './merch_view';
...
export class MerchStorePage {
  ...
  private usdLabel(usd: number): string {
    return `$${formatNumber(usd, { maximumFractionDigits: 2 })}`;
  }
    ...
    const rows = view.orderRows
      .map((row) => {
        ...
        return (
          ...
          `<span class="merch-order-items">${esc(row.itemsLabel)}</span>` +
  ...
  /**
   * Fold a checkout outcome into the page state. A paid claudium order or a
   * settled sol order clears the cart; a stripe order clears it too (the embedded
   * checkout owns the payment from here; the webhook settles it service-side).
   * Anything else leaves the cart intact with a typed notice.
   */
  private applyOutcome(outcome: MerchCheckoutOutcome): void {
    const { checkout, settlement } = outcome;
    if (!checkout.ok || !checkout.orderId) {
      this.noticeKey =
        checkout.reason === 'invalid_request'
          ? 'hudChrome.merch.shippingIncomplete'
          : 'hudChrome.merch.checkoutUnavailable';
      return;
    }
    if (checkout.rail === 'sol' && settlement && !settlement.settled) {
      this.noticeKey = 'hudChrome.merch.checkoutNotSettled';
      return;
    }
    this.cart.clear();
    this.lastOrderId = checkout.orderId;
    this.noticeKey = null;
  }
}
