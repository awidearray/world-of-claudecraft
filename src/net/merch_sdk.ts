...
export interface MerchSigners {
  ...
  nativeSignAndSend?(transactionBase64: string): Promise<string>;
}
...
export async function startMerchCheckout(
  ...
  if (input.rail === 'sol') {
    ...
    const signature = await signers.nativeSignAndSend(checkout.native.transactionBase64);
    ...
    // two typed results without re-deriving the retry policy.
