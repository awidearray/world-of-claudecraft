...
export interface NearbyInteractionWorld {
  ...
  // The RiverBoat casino seam: a gangway/exit portal object OR a croupier NPC
  // routes here (the sim decides board/disembark/open-station by templateId).
...
export function tryNearbyInteraction(
  ...
  if (bestObject !== null) {
    ...
    if (object.templateId === 'dungeon_door' && object.dungeonId) {
      ...
      object.templateId === 'riverboat_gangway' ||
      object.templateId === 'riverboat_exit'
    ) {
      // The RiverBoat gangway boards / the deck exit disembarks; the sim routes
      // by templateId (walk-in triggers fire the same seam).
      world.casinoInteract(bestObject);
      return true;
    } else {
  ...
  if (bestNpc !== null) {
    ...
    } else if (npc.templateId?.startsWith('riverboat_')) {
      // A RiverBoat croupier opens its game station window, not a quest dialog.
